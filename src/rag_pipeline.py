...
class LocalRAGPipeline:
    ...
    def build_knowledge_graph(self, documents: List[Document]):
        ...
        for doc in documents:
            ...
            for other_doc in documents:
                if (doc.doc_id != other_doc.doc_id and 
                    doc.metadata.get('source') == other_doc.metadata.get('source')):
                    self.knowledge_graph.add_edge(
                        doc.doc_id,
                        other_doc.doc_id,
        ...
        print(f"Knowledge graph built with {self.knowledge_graph.number_of_nodes()} nodes "
    ...
    def hybrid_search(
        ...
        graph_scores = {doc.doc_id: 1.0 / (i + 1) for i, doc in enumerate(graph_results)}
...
Answer:"""
    ...
    def save(self):
        ...
        if self.vector_index is not None:
            faiss.write_index(self.vector_index, str(self.storage_path / "vector_index.faiss"))
        
        # Save knowledge graph
        nx.write_gpickle(self.knowledge_graph, self.storage_path / "knowledge_graph.gpickle")
    ...
    def load(self):
        ...
        if (self.storage_path / "vector_index.faiss").exists():
            self.vector_index = faiss.read_index(str(self.storage_path / "vector_index.faiss"))
        
        # Load knowledge graph
        if (self.storage_path / "knowledge_graph.gpickle").exists():
            self.knowledge_graph = nx.read_gpickle(self.storage_path / "knowledge_graph.gpickle")
